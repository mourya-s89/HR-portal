  ...
  return (
    <div className="space-y-8 animate-fade-in-up pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-[#111827] tracking-tight font-extrabold">Payroll Ledger</h1>
          <p className="text-muted-foreground mt-1 font-medium">Distribute monthly compensation statements to the team.</p>
        </div>
        {!showUpload && (
          <button 
            onClick={() => setShowUpload(true)}
            className="flex items-center gap-2 px-6 py-3 rounded-[24px] bg-emerald-600 text-white font-bold shadow-xl hover:bg-emerald-700 transition-all active:scale-95"
          >
      ...
      {showUpload && (
        <div className=" rounded-[32px] p-8 border border-white bg-white/80 animate-fade-in-up shadow-2xl">
           ...
           <form onSubmit={handleUpload} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                 <div className="space-y-2">
                    ...
                    <select 
                      ...
                      className="w-full bg-slate-50 border border-slate-100 rounded-[24px] px-5 py-3.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 font-bold"
                    >
              ...
              <div className="space-y-2">
                 ...
                 <div className="border-2 border-dashed border-slate-100 rounded-3xl p-10 flex flex-col items-center justify-center bg-slate-50 hover:bg-emerald-50/30 hover:border-emerald-200 transition-all cursor-pointer relative group">
                    <input 
                      type="file" 
                      required
              ...
              <div className="flex justify-end gap-3 pt-4">
                 ...
                 <button 
                  type="submit" 
                  disabled={submitting}
                  className="px-10 py-4 rounded-[24px] bg-emerald-600 text-white font-bold hover:bg-emerald-700 transition-all shadow-xl active:scale-95 flex items-center gap-2"
                 >
      ...
      <div className="space-y-6">
        <div className="relative">
           ...
           <input 
             ...
             className="w-full bg-white border border-slate-100 rounded-[24px] pl-12 pr-6 py-4.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 shadow-sm font-medium"
           />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
           {loading ? (
              Array.from({ length: 8 }).map((_, i) => (
                 <div key={i} className=" h-52 rounded-[32px] animate-pulse bg-white border border-slate-100" />
              ))
           ) : filteredPayslips.length > 0 ? (
             filteredPayslips.map((p) => (
               <div key={p._id} className=" group p-6 rounded-[32px] border border-slate-100 bg-white hover:border-emerald-100 transition-all shadow-sm hover:shadow-xl flex flex-col relative overflow-hidden">
                  <div className="flex items-start justify-between mb-4">
                     <div className="w-12 h-12 rounded-[24px] bg-emerald-50 text-emerald-500 flex items-center justify-center font-bold text-lg group-hover:bg-emerald-600 group-hover:text-white transition-all shadow-inner">
                        <Receipt className="w-6 h-6" />
                     </div>
                     <div className="text-right">
                        <p className="text-[14px] font-black text-[#111827] leading-tight">{p.year}</p>
                        <p className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest">{getMonthName(p.month)}</p>
                     </div>
                  </div>

                  <h3 className="font-extrabold text-[#111827] text-sm mb-1 truncate group-hover:text-emerald-600 transition-colors" title={p.userId?.name}>
                     {p.userId?.name}
                  </h3>
                  ...
                  <div className="mt-auto pt-4 border-t border-slate-50 flex items-center gap-2">
                     <a 
                       ...
                       target="_blank"
                       className="flex-1 py-2.5 rounded-2xl bg-[#111827] text-white text-[11px] font-bold text-center hover:bg-slate-800 transition-all flex items-center justify-center gap-2"
                     >
                        <Eye className="w-3.5 h-3.5" /> View
                     </a>
                     <button 
                       onClick={() => deletePayslip(p._id)}
                       className="p-2.5 rounded-2xl bg-slate-50 text-slate-400 hover:bg-rose-50 hover:text-rose-600 transition-all shadow-sm group-hover:opacity-100"
                       title="Delete"
                     >
