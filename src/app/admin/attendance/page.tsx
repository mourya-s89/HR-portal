...
export default function AdminAttendancePage() {
  ...
  const [filterType, setFilterType] = useState<"all" | "present" | "late" | "absent">("all");
  ...
  const filteredEmployees = employees.filter(emp => {
    ...
    if (filterType === "late") return rec?.isLate === true;
    if (filterType === "absent") return !rec;
    return true;
  });
    ...
    <div className="space-y-8 animate-fade-in-up pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          <Link href="/admin/dashboard" className="p-3 rounded-[24px] bg-slate-50 text-slate-400 hover:text-[#111827] border border-slate-100 transition-all">
            <ArrowLeft className="w-5 h-5" />
          </Link>
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-[#111827]">Workforce Participation</h1>
            <p className="text-muted-foreground mt-1 font-medium">Monitoring real-time attendance and site locations.</p>
          </div>
        </div>

        <div className="flex items-center gap-3">
          <div className="flex items-center bg-white border border-slate-100 rounded-[24px] p-1.5 shadow-sm">
            <button 
              onClick={() => {
          ...
          <button onClick={fetchData} className="p-3.5 rounded-full bg-[#111827] text-white shadow-lg hover:bg-slate-800 active:scale-95 transition-all">
            <RefreshCcw className={cn("w-5 h-5", loading && "animate-spin")} />
          </button>
        </div>
      </div>

      {/* Stats Bar with Selection */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <button 
          onClick={() => setFilterType("all")}
          className={cn(
            "p-6 rounded-[28px] border bg-white shadow-sm flex items-center justify-between transition-all active:scale-95",
            filterType === "all" ? "ring-2 ring-[#1E6DEB] border-transparent" : "border-slate-100 hover:border-indigo-200"
          )}
        >
          ...
          <div className="p-3.5 rounded-[24px] bg-[#EDF2FE] text-[#1E6DEB]"><Users className="w-6 h-6" /></div>
        </button>
        ...
        <button 
          ...
          className={cn(
            "p-6 rounded-[28px] border bg-white shadow-sm flex items-center justify-between transition-all active:scale-95",
            filterType === "present" ? "ring-2 ring-emerald-500 border-transparent" : "border-slate-100 hover:border-emerald-200"
          )}
        >
          <div className="text-left">
          ...
          <div className="p-3.5 rounded-[24px] bg-emerald-50 text-emerald-500"><CheckCircle2 className="w-6 h-6" /></div>
        </button>
        ...
        <button 
          ...
          className={cn(
            "p-6 rounded-[28px] border bg-white shadow-sm flex items-center justify-between transition-all active:scale-95",
            filterType === "late" ? "ring-2 ring-amber-500 border-transparent" : "border-slate-100 hover:border-amber-200"
          )}
        >
          <div className="text-left">
          ...
          <div className="p-3.5 rounded-[24px] bg-amber-50 text-amber-500"><Clock className="w-6 h-6" /></div>
        </button>
        ...
        <button 
          ...
          className={cn(
            "p-6 rounded-[28px] border bg-white shadow-sm flex items-center justify-between transition-all active:scale-95",
            filterType === "absent" ? "ring-2 ring-rose-500 border-transparent" : "border-slate-100 hover:border-rose-200"
          )}
        >
          <div className="text-left">
          ...
          <div className="p-3.5 rounded-[24px] bg-rose-50 text-rose-500"><UserX className="w-6 h-6" /></div>
        </button>
      </div>
      ...
      <div className="space-y-6">
        <div className="relative">
          ...
          <input 
            ...
            className="w-full bg-white border border-slate-100 rounded-[20px] pl-11 pr-4 py-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#EDF2FE] shadow-sm"
          />
        </div>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                    ...
                    return (
                      <tr key={emp._id} className="hover:bg-slate-50 transition-colors group">
                        <td className="px-8 py-5">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-2xl bg-slate-100 text-slate-500 flex items-center justify-center font-bold text-sm shadow-sm">
                              {emp.name[0]}
                            </div>
                            <div>
                               <p className="text-sm font-extrabold text-[#111827] leading-tight">{emp.name}</p>
                        ...
                        <td className="px-6 py-5">
                          <p className="text-sm font-bold text-[#111827]">{rec?.totalHours ? `${rec.totalHours}h` : "—"}</p>
                        </td>
                        <td className="px-6 py-5">
                          <div className="flex justify-center gap-2">
                             {rec?.checkInLocation?.address && (
                               <div className="relative group/loc">
                                 <button className="p-2 rounded-2xl bg-slate-100 text-slate-500 hover:bg-slate-900 hover:text-white transition-all">
                                   <MapPin className="w-4 h-4" />
                                 </button>
                                 <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-2.5 bg-[#111827] text-white text-[10px] rounded-2xl opacity-0 invisible group-hover/loc:opacity-100 group-hover/loc:visible transition-all z-20 shadow-xl">
                        ...
                        <td className="px-8 py-5 text-right">
                          <button 
                            ...
                            className="p-3 rounded-2xl bg-slate-100 text-slate-400 hover:bg-[#1E6DEB] hover:text-white transition-all shadow-sm active:scale-95"
                          >
                  ...
                  <tr><td colSpan={7} className="px-8 py-20 text-center"><p className="text-slate-400 font-bold">No matching employees found for this filter.</p></td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {showEditModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 animate-fade-in">
          <div className="bg-white rounded-[32px] w-full max-max-lg p-10 shadow-2xl animate-fade-in-up border border-slate-100 overflow-hidden">
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-xl font-extrabold text-[#111827] tracking-tight flex items-center gap-3">
                 <div className="p-2.5 rounded-2xl bg-[#EDF2FE] text-[#1E6DEB]"><Edit3 className="w-5 h-5" /></div>
            ...
            <form onSubmit={handleUpdateRecord} className="space-y-8">
              <div className="p-4 rounded-[24px] bg-[#EDF2FE]/50 border border-[#EDF2FE] flex items-center gap-4">
                 <div className="w-10 h-10 rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 flex items-center justify-center font-bold text-slate-600">
                    {employees.find(e => e._id === editingRecord?.userId)?.name?.[0]}
                 </div>
                 <div>
                    <p className="text-sm font-bold text-[#111827]">{employees.find(e => e._id === editingRecord?.userId)?.name}</p>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <select 
                    ...
                    className="w-full bg-slate-50 border border-slate-100 rounded-[24px] px-4 py-3.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#EDF2FE] font-bold text-slate-700"
                  >
                ...
                <div className="space-y-3 pt-6 flex flex-col justify-center">
                  <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-[24px] border border-slate-100">
                    <input 
                       type="checkbox" 
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-50 border border-slate-100 rounded-[24px] px-4 py-3.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#EDF2FE] font-bold text-slate-700 uppercase" 
                  />
                </div>
              ...
              <div className="flex gap-4 pt-4">
                 <button 
                   ...
                   className="flex-1 px-8 py-4 rounded-[24px] bg-slate-100 text-slate-600 font-bold hover:bg-slate-200 active:scale-95 transition-all text-sm"
                 >
                   Cancel
                 </button>
                 <button 
                    type="submit" 
                    disabled={submitting}
                    className="flex-[2] px-8 py-4 rounded-full bg-[#111827] text-white font-bold shadow-xl shadow-slate-200 hover:bg-slate-800 active:scale-95 transition-all flex items-center justify-center gap-3 text-sm"
                 >
