...
const ROLE_COLORS: Record<string,string> = { Admin:"bg-red-500/10 text-red-500", HR:"bg-violet-500/10 text-violet-500", Employee:"bg-[#1E6DEB]/10 text-[#1E6DEB]" };
  ...
  return (
    <div className="space-y-8 animate-fade-in-up pb-20">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-[#111827] tracking-tight font-extrabold text-[#111827]">Employee Directory</h1>
        ...
        {!showForm && (
          <button onClick={() => setShowForm(true)}
            className="flex items-center gap-2 px-6 py-3 rounded-full bg-[#111827] text-white font-bold shadow-lg hover:bg-slate-800 transition-all active:scale-95">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in">
        {[
          ...
          { label: "Employees", count: employees.filter(e => e.role === "Employee").length, role: "Employee", color: "from-[#1E6DEB] to-blue-500", icon: "👥" },
        ].map((item, idx) => (
          <button 
            key={idx} 
            ...
            className={cn(
              ...
              roleFilter === item.role 
                ...
                : "bg-white border-slate-100 shadow-sm hover:shadow-lg hover:border-[#EDF2FE]"
            )}
          >
            ...
            <div className="flex items-center justify-between relative z-10">
              <div className="space-y-1">
                ...
                <p className={cn("text-3xl font-black", roleFilter === item.role ? "text-white" : "text-[#111827]")}>{item.count}</p>
                <p className={cn("text-[9px] font-bold mt-2 flex items-center gap-1", roleFilter === item.role ? "text-indigo-400" : "text-[#1E6DEB] opacity-0 group-hover:opacity-100 transition-all")}>
            ...
            {roleFilter === item.role && (
               <div className="absolute bottom-0 left-0 w-full h-1 bg-[#1E6DEB]" />
            )}
          </button>
        ))}
      </div>
      ...
      {/* Form Section */}
      {showForm && (
        <div className=" rounded-[2rem] p-8 border border-white/10 bg-white animate-fade-in-up shadow-xl overflow-hidden relative">
          <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-indigo-500 to-violet-500" />
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-xl font-extrabold text-[#111827] tracking-tight flex items-center gap-3">
              <div className="p-2.5 rounded-2xl bg-[#EDF2FE] text-[#1E6DEB]">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              ...
              <select value={form.role} onChange={e=>setForm({...form,role:e.target.value})} className="w-full bg-slate-50 border border-slate-100 rounded-[24px] px-4 py-3.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#EDF2FE] font-bold text-slate-700">
            ...
            <div className="md:col-span-2 lg:col-span-3 flex gap-4 justify-end pt-4">
              <button type="button" onClick={handleCloseForm} className="px-8 py-3.5 rounded-[24px] bg-slate-100 text-slate-600 font-bold hover:bg-slate-200 transition-all active:scale-95">Cancel</button>
              <button type="submit" disabled={loading} className="px-10 py-3.5 rounded-full bg-[#111827] text-white font-bold hover:bg-slate-800 transition-all active:scale-95 disabled:opacity-60 flex items-center gap-2">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search by name, email or employee ID..."
            className="w-full bg-white border border-slate-100 rounded-[24px] pl-11 pr-4 py-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#EDF2FE] font-medium shadow-sm"/>
        </div>
        <div className="relative min-w-[150px]">
           <Users className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
           <select value={roleFilter} onChange={e=>setRoleFilter(e.target.value)} className="w-full appearance-none bg-white border border-slate-100 rounded-[24px] pl-11 pr-10 py-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#EDF2FE] font-bold text-slate-600 shadow-sm cursor-pointer">
              {["All", "Admin", "HR", "Employee"].map(r => <option key={r} value={r}>{r}</option>)}
           </select>
        </div>
        <div className="relative min-w-[180px]">
           <Filter className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
           <select value={deptFilter} onChange={e=>setDeptFilter(e.target.value)} className="w-full appearance-none bg-white border border-slate-100 rounded-[24px] pl-11 pr-10 py-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#EDF2FE] font-bold text-slate-600 shadow-sm cursor-pointer">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filtered.map((emp) => (
          <div key={emp._id} className=" p-6 rounded-[2rem] border border-white/10 hover:border-[#EDF2FE] transition-all group space-y-5 bg-white shadow-sm hover:shadow-xl relative overflow-hidden">
            <div className="flex items-start gap-4">
              <div className="w-14 h-14 rounded-[24px] bg-[#1E6DEB] text-white flex items-center justify-center text-xl font-black shrink-0 shadow-lg shadow-indigo-100 ring-4 ring-indigo-50">
                {emp.name[0].toUpperCase()}
              </div>
              <div className="min-w-0 flex-1">
                <p className="font-black text-[#111827] text-lg truncate leading-tight">{emp.name}</p>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-slate-50">
              ...
              <div className="flex items-center gap-2">
                 <button onClick={() => handleEdit(emp)} className="p-2.5 rounded-2xl bg-slate-50 text-slate-400 hover:bg-slate-900 hover:text-white transition-all shadow-sm active:scale-95">
                    <Edit2 className="w-4 h-4" />
                 </button>
                 <button onClick={() => handleDelete(emp._id)} className="p-2.5 rounded-2xl bg-slate-50 text-slate-400 hover:bg-rose-500 hover:text-white transition-all shadow-sm active:scale-95">
        ...
        {filtered.length === 0 && (
          <div className="col-span-full flex flex-col items-center justify-center py-20 bg-slate-50 rounded-[3rem] border-2 border-dashed border-slate-100">
