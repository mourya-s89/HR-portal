...
export default function PayslipsPage() {
  ...
  return (
    <div className="space-y-8 animate-fade-in-up">
      <div>
        <h1 className="text-3xl font-extrabold tracking-tight text-[#111827] tracking-tight font-extrabold">Payslips</h1>
      ...
      {Object.entries(grouped).sort(([a],[b]) => +b - +a).map(([year, slips]) => (
        <div key={year} className=" rounded-3xl p-8 border border-white/5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {slips.sort((a,b) => (b.month||0)-(a.month||0)).map((slip, i) => (
              <div key={i} className="p-5 rounded-[24px] bg-muted/30 border border-white/5 hover:bg-muted/50 transition-all group">
                <div className="flex items-start justify-between">
                  ...
                  <div className="p-2 rounded-2xl bg-violet-500/10 text-violet-400 group-hover:bg-violet-500/20 transition-colors">
                    <Receipt className="w-4 h-4" />
                  </div>
                </div>
                <a href={`/api/documents/${slip._id}`} target="_blank" rel="noopener noreferrer"
                  className="mt-4 w-full flex items-center justify-center gap-2 py-2.5 rounded-[24px] bg-[#1E6DEB]/10 text-indigo-400 text-sm font-medium hover:bg-[#1E6DEB]/20 transition-colors">
      ...
      {payslips.length === 0 && (
        <div className=" rounded-3xl p-16 border border-white/5 flex flex-col items-center text-center text-muted-foreground space-y-4">
