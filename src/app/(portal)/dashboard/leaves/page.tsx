    ...
    <div className="space-y-8 animate-fade-in-up">
      <div className="flex items-start justify-between">
        ...
        <button onClick={() => setShowForm(!showForm)}
          className="flex items-center gap-2 px-5 py-3 rounded-[24px] bg-gradient-to-r from-[#1E6DEB] to-blue-600 text-white font-semibold shadow-lg shadow-indigo-600/20 hover:opacity-90 transition-all active:scale-95">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className=" p-6 rounded-3xl border border-white/5">
          ...
          <p className="text-4xl font-extrabold tracking-tight text-indigo-400">{clBalance}</p>
        ...
        <div className=" p-6 rounded-3xl border border-white/5">
          ...
          <p className="text-4xl font-extrabold tracking-tight text-orange-400">{leaves.filter(l => l.isLOP && l.status === "Approved").reduce((a,l) => a + l.days, 0)}</p>
        ...
        <div className=" p-6 rounded-3xl border border-white/5">
          ...
          <p className="text-4xl font-extrabold tracking-tight text-yellow-400">{leaves.filter(l => l.status === "Pending").length}</p>
      ...
      {/* Apply Form */}
      {showForm && (
        <div className=" rounded-3xl p-8 border border-white/5 animate-fade-in-up">
          <h2 className="text-lg font-bold mb-6">Apply for Leave</h2>
          {clBalance === 0 && (
            <div className="flex items-center gap-2 p-4 rounded-[24px] bg-orange-500/10 border border-orange-500/20 text-orange-400 text-sm mb-6">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <select value={form.type} onChange={e => setForm({...form, type: e.target.value})}
                className="w-full bg-muted/30 border border-white/10 rounded-[24px] px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50">
            ...
            <div className="md:col-span-2 flex gap-4 justify-end">
              <button type="button" onClick={() => setShowForm(false)} className="px-6 py-3 rounded-[24px] bg-muted/50 text-muted-foreground font-medium hover:bg-muted transition-colors">Cancel</button>
              <button type="submit" disabled={loading} className="px-6 py-3 rounded-[24px] bg-gradient-to-r from-[#1E6DEB] to-blue-600 text-white font-semibold hover:opacity-90 transition-all active:scale-95 disabled:opacity-60">
      ...
      <div className=" rounded-3xl p-8 border border-white/5 space-y-4">
        ...
        <div className="space-y-3">
          {leaves.map((leave, i) => (
            <div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-[24px] bg-muted/30 border border-white/5 hover:bg-muted/50 transition-colors">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className={cn("px-2 py-0.5 rounded-md text-[10px] font-bold uppercase", leave.isLOP ? "bg-orange-500/10 text-orange-400" : "bg-[#1E6DEB]/10 text-indigo-400")}>{leave.isLOP ? "LOP" : leave.type}</span>
              ...
              <span className={cn("px-3 py-1.5 rounded-2xl text-xs font-bold self-start sm:self-center", STATUS_STYLE[leave.status])}>{leave.status}</span>
            </div>
          ))}
